...
RegisterNetEvent('hw_garage:setVehicleProperties', function(netId, data)
    local timeout = 10000
...
function SpawnVehicle(args)
    ...
    local netId = lib.callback.await('hw_garage:takeOutVehicle', false, index, props.plate, type)
...
local function openGarageVehicles(args)
    ...
    local vehicles = lib.callback.await('hw_garage:getOwnedVehicles', false, index, society)
        ...
        local option = {
            ...
            onSelect = vehicle.state == 'in_garage' and SpawnVehicle or function()
                if vehicle.state == 'out_garage' then
                    local coords = lib.callback.await('hw_garage:getVehicleCoords', false, vehicle.plate)
...
local function saveVehicle(vehicle)
    ...
    local result = lib.callback.await('hw_garage:saveVehicle', false, props, netId)
...
local function retrieveVehicle(args)
    ...
    lib.requestModel(props.model)
    local type = getVehicleType(props.model)
    local success, netId = lib.callback.await('hw_garage:retrieveVehicle', false, index, props.plate, type)
...
local function openImpoundVehicles(args)
    local index, society in args
    local vehicles = lib.callback.await('hw_garage:getImpoundedVehicles', false, index, society)
